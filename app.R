...
A sample CSV file for herring can be loaded. To perform the necessary calculations, the 
app requires the following columns:
<ul>
  <li>Year</li>
  <li>Age</li>
...
values are not allowed in any of these columns. Additionally, the app requires 
parameters to describe the stock-recruit relationship. You can specify this relationship 
using either a Beverton–Holt model (R = αS/(1 + βS)) or a Ricker model (R = αSe<sup>−βS</sup>), 
based on the parameters α and β (alpha and beta). Alternatively, if no 
stock–recruit relationship is assumed, you can assume a constant recruitment value.</p>
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Data Upload", class = "tab-content", style = "height: 70vh;",
             ...
             fluidRow(
               column(width = 2,
                      wellPanel(div(tags$label("Load Sample Data"),
                                    actionButton("loadSample", "'herring.csv'"))),
                      wellPanel(
             ...
             fluidRow(
               column(width = 2,
                      ...
                      conditionalPanel(
                        ...
                        numericInput("alpha", "α (Alpha) Value:", value = 0.09, 
                                     step = 0.01, min = 0),
                        numericInput("beta", "β (Beta) Value:", value = 0.03, 
