...
MP_entirets <- function (x, Data, reps = 1){
  ...
  if(y%%5==1)
  {
    ...
    EM_list$EntireTS[[((y - 1) %/% 5) + 1,x]] <<- mySCA
  }
...
MP_mixed_BPA <- function (x, Data, reps = 1){
  ...
  if(y%%5==1)
  {
    ...
    EM_list$MixedBPA[[((y - 1) %/% 5) + 1,x]] <<- mySCA
  }
