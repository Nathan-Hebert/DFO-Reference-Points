...
fig_width_l <- 16; fig_height_l <- 11 # For the LRP/USR figures
...
decrease_coord_lrp <- coord_cartesian(xlim = c(51,100), ylim = c(0,30))
...
(growthup_lowF_plots[["Median_SSB_LRPs"]] + 
  ...
  (decreaseM_lowF_plots[["Median_SSB_LRPs"]] + theme_no_y +
     ggtitle("Increasing productivity\n(M), non-rebuilding") + increase_coord_lrp) +
  (decreaseM_highF_plots[["Median_SSB_LRPs"]] + theme_no_y +
     ggtitle("Increasing productivity\n(M), rebuilding") + increase_coord_lrp) +
  ...
  (increaseM_lowF_plots[["Median_SSB_LRPs"]] + theme_no_y +
     ggtitle("Decreasing productivity\n(M), non-rebuilding") + decrease_coord_lrp) +
  (increaseM_highF_plots[["Median_SSB_LRPs"]] + theme_no_y +
     ggtitle("Decreasing productivity\n(M), rebuilding") + decrease_coord_lrp) +
  plot_layout(nrow = 3, guides = "collect", heights = c(1,0.1,1)) &
  theme(plot.title = element_text(size = font_size, face = "bold"),
        text = element_text(size = font_size),
        legend.text = element_text(size = font_size),
        legend.position = "bottom", legend.key.size = unit(1.5, "cm"))
ggsave(paste0(getwd(),"/Figs/MSE/combined_MSE_LRPs_USRs.jpeg"), plot=last_plot(), 
       width=fig_width_l, height=fig_height_l, units="in")
