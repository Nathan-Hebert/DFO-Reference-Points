...
# Set axis limits for other plots (increasing scenarios and decreasing scenarios)
coord_LRP <- coord_cartesian(xlim = c(46,NA), ylim = c(0,1))
increase_coord_SSB <- coord_cartesian(xlim = c(46,NA), ylim = c(0,57))
increase_coord_F <- coord_cartesian(xlim = c(46,NA), ylim = c(0,0.54))
increase_coord_catch <- coord_cartesian(xlim = c(46,NA), ylim = c(0,15))
decrease_coord_SSB <- coord_cartesian(xlim = c(46,NA), ylim = c(0,42))
decrease_coord_F <- coord_cartesian(xlim = c(46,NA), ylim = c(0,0.25))
decrease_coord_catch <- coord_cartesian(xlim = c(46,NA), ylim = c(0,4.5))

# Arrange increasing scenarios' SSB, F, catch, P(catch = 0 kt), and P(SSB>LRP) 
# into a grid and save
(growthup_lowF_plots[["Median_SSB"]] + guides(col = guide_legend(nrow = 1)) +
    theme_no_x + increase_coord_SSB +
    ggtitle("Increasing productivity\n(G & R), non-rebuilding")) +
  (growthup_highF_plots[["Median_SSB"]] + 
    theme_no_xy + guides(color = "none") + increase_coord_SSB + 
    ggtitle("Increasing productivity\n(G & R), rebuilding")) + 
  ...
  (decreaseM_highF_plots[["Prob_Catch"]] + theme_no_xy + guides(color = "none") + 
     coord_LRP) +
  (growthup_lowF_plots[["Prob_SSB_LRP"]] + guides(color = "none") + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + coord_LRP) + 
  (growthup_highF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
    coord_LRP) +
  (decreaseM_lowF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
    coord_LRP) +
  (decreaseM_highF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
    coord_LRP) +
...
# Arrange decreasing scenarios' SSB, F, catch, P(catch = 0 kt), and P(SSB>LRP) 
# into a grid and save
(growthdown_lowF_plots[["Median_SSB"]] + guides(col = guide_legend(nrow = 1)) + 
    theme_no_x + ggtitle("Decreasing productivity\n(G & R), non-rebuilding") + 
    decrease_coord_SSB) +
  (growthdown_highF_plots[["Median_SSB"]] + 
     theme_no_xy + ggtitle("Decreasing productivity\n(G & R), rebuilding") + 
     guides(color = "none") + decrease_coord_SSB) +
  ...
  (growthdown_lowF_plots[["Prob_SSB_LRP"]] + guides(color = "none") + 
     theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + coord_LRP) + 
  (growthdown_highF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
     coord_LRP) +
  (increaseM_lowF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
     coord_LRP) +
  (increaseM_highF_plots[["Prob_SSB_LRP"]] + theme_no_y + guides(color = "none") +
     coord_LRP) +
  plot_layout(guides = "collect", ncol = 4, nrow = 5) &
  theme(plot.title = element_text(size = font_size, face = "bold"),
        text = element_text(size = font_size),
...
# Set axis limits for plots (increasing scenarios and decreasing scenarios)
increase_coord_SSB_zoomout <- coord_cartesian(ylim = c(0,64))
increase_coord_F_zoomout <- coord_cartesian(ylim = c(0,0.74))
increase_coord_catch_zoomout <- coord_cartesian(ylim = c(0,13))
decrease_coord_SSB_zoomout <- coord_cartesian(ylim = c(0,90))
decrease_coord_F_zoomout <- coord_cartesian(ylim = c(0,0.54))
decrease_coord_catch_zoomout <- coord_cartesian(ylim = c(0,11))

# Arrange increasing scenarios' SSB, F, catch, P(catch = 0 kt), and P(SSB>LRP) 
# into a grid --- now zoomed out to show all historical years --- and save
(growthup_lowF_plots[["Median_SSB"]] + guides(col = guide_legend(nrow = 1)) +
...
(growthdown_lowF_plots[["Median_SSB"]] + guides(col = guide_legend(nrow = 1)) +
    theme_no_x + ggtitle("Decreasing productivity\n(G & R), non-rebuilding") + 
    decrease_coord_SSB_zoomout) +
  (growthdown_highF_plots[["Median_SSB"]] + 
     theme_no_xy + ggtitle("Decreasing productivity\n(G & R), rebuilding") + 
     guides(color = "none") + decrease_coord_SSB_zoomout) +
  (increaseM_lowF_plots[["Median_SSB"]] + 
     theme_no_xy + ggtitle("Decreasing productivity\n(M), non-rebuilding") + 
     guides(color = "none") + decrease_coord_SSB_zoomout) +
  (increaseM_highF_plots[["Median_SSB"]] + 
