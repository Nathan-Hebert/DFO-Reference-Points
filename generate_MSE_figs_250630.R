...
font_size <- 16
width <- 8 # Figure width in inches (except boxplot panel figure)
height <- 6 # Figure height in inches (except boxplot panel figure)
width_boxplot <- 8.8 # Boxplot panel figure width in inches
height_boxplot <- 10.2 # Boxplot panel figure height in inches
...
LQ <- 0.05; UQ <- 0.95 # Quantiles for plotting alongside median
...
# Generates boxplots for a performance metric over either the ST or LT
plot_boxplot <- function(data = boxplot_DF, period_chosen = "ST", y_var, ylabel = "", 
                         text_size = font_size) {
  data %>%
    filter(period == period_chosen, !MP %in% c("NFref", NA)) %>%
    ggplot(aes(x = MP, fill = MP, y = .data[[y_var]])) + 
    ...
    labs(x = "", y = ylabel) + scale_x_discrete(labels = MP_labs) +
    scale_fill_manual(values = palette, labels = MP_labs)
}
...
plot_ts <- function(proj_df = PlotDF, hist_df = hist, proj_col, hist_col, line_wid = lwd, 
                    ...
                    text_size = font_size, show_historical = FALSE, HCR_lab = "")
{
  ...
  return(ggplot()  +
           ...
           xlab("Year") + labs(col = HCR_lab) + ylab(ylab) +
           theme_classic() + theme(text = element_text(size = text_size),
...
boxplot_DF <- DFRP %>%
  mutate(period = ifelse(Yr < 6, "ST", ifelse(Yr > 25, "LT", NA)),
         SSB_SSBMSYFirst5 = 0.4*SSB/LRP_First5)
boxplot_DF_AAVY <- pivot_longer(AAVY_df, cols = c("ST_AAVY","LT_AAVY")) %>% 
  mutate(period = substr(name, 1,2))
...
# Start setting up dataframe used to illustrate difference in RPs/SRRs between OM and EM
rp_bias_df <- expand.grid(EM_fit = 1:10, sim = 1:OM@nsim,
                          period = c("Initial", "Recent", "Full"))
rp_bias_df$bmsy_ratio <- NA
rp_bias_df$fmsy_ratio <- NA
rp_bias_df$alpha_ratio <- NA
rp_bias_df$beta_ratio <- NA
period_info <- list(
  "Initial" = list(SSB = "SSB_MSY_First5", FM = "F_MSY_First5", srr = "First5",
                   LRP = "LRP_First5", FMSY = "FMSY_First5", MP = "MP_first5"),
  "Recent"  = list(SSB = "SSB_MSY_Last5", FM = "F_MSY_Last5", srr = "Last5",
                   LRP = "LRP_Last5", FMSY = "FMSY_Last5", MP = "MP_last5"),
  "Full" = list(SSB = "SSB_MSY_EntireTS", FM = "F_MSY_EntireTS", srr = "EntireTS",
                LRP = "LRP_EntireTS", FMSY = "FMSY_EntireTS", MP = "MP_entirets"))

# Loop through simulations and EM fits, and fill in the above dataframe with EM-OM ratios for 
# FMSY, BMSY, SRR alpha, SRR beta
for (i in 1:OM@nsim) {
  for (j in 1:10) {
    idx <- 1 + (j - 1) * 5
    for (period in names(period_info)) {
      info <- period_info[[period]]
      row_filter <- rp_bias_df$EM_fit == j & rp_bias_df$sim == i & rp_bias_df$period == period
      lrp_val <- DF[[info$LRP]][DF$Sim == i & DF$Yr == idx & DF$MP == info$MP]
      fmsy_val <- DF[[info$FMSY]][DF$Sim == i & DF$Yr == idx & DF$MP == info$MP]
      alpha_val <- DF[["alpha_sim"]][DF$Sim == i & DF$Yr == idx & DF$MP == info$MP]
      beta_val <- DF[["beta_sim"]][DF$Sim == i & DF$Yr == idx & DF$MP == info$MP]
      rp_bias_df[row_filter, "bmsy_ratio"] <- CP_list[[info$SSB]][idx, i] / (lrp_val / 0.4)
      rp_bias_df[row_filter, "fmsy_ratio"] <- CP_list[[info$FM]][idx, i] / (fmsy_val)
      rp_bias_df[row_filter, "alpha_ratio"] <- unlist(EM_list[[info$srr]][j, i][[1]]["alpha"]) / (alpha_val)
      rp_bias_df[row_filter, "beta_ratio"] <- unlist(EM_list[[info$srr]][j, i][[1]]["beta"]) / (beta_val)
    }
  }
}
...
# Plot illustrating the OM-EM ratio for FMSY and BMSY
temp_df <- rp_bias_df %>%
  pivot_longer(cols = c("bmsy_ratio","fmsy_ratio")) %>%
  mutate(rp = substr(name,1,4), ratio = value) %>%
  group_by(EM_fit, period, rp) %>%
  summarize(ratio_median = median(ratio),
            ratio_lq = quantile(ratio, probs = LQ),
            ratio_uq = quantile(ratio, probs = UQ))
ymax <- max(temp_df$ratio_uq)*1.1
plot_list[["bias_rps"]] <- 
  ggplot(data = temp_df, aes(x = EM_fit, y = ratio_median, ymax = ratio_lq, 
                             ymin = ratio_uq, col = rp)) +
  geom_errorbar() + geom_point() +
  coord_cartesian(ylim = c(0.45, ymax), expand = FALSE) +
  scale_x_continuous(breaks = c(1:10)) +
  facet_wrap(~period, ncol = 1) +
  xlab("Estimation model fit") + ylab("Ratio (EM/OM)") +
  scale_color_manual("", values = c("red","blue"), 
                     labels = c(expression(SSB[MSY]), expression(F[MSY]))) +
  theme_classic() +
  theme(text = element_text(size = font_size), 
        strip.text = element_text(size = font_size),
        legend.text = element_text(size = font_size),
        panel.grid.major = element_line(color = "grey80", size = 0.5))
ggsave(paste0(getwd(),"/Figs/MSE/",OM_name,"/bias_rps.jpeg"), plot=last_plot(), 
       width=width, height=height, units="in")

# Plot illustrating the OM-EM ratio for alpha and beta
temp_df <- rp_bias_df %>%
  pivot_longer(cols = c("alpha_ratio","beta_ratio")) %>%
  mutate(param = substr(name,1,4), ratio = value) %>%
  group_by(EM_fit, period, param) %>%
  summarize(ratio_median = median(ratio),
            ratio_lq = quantile(ratio, probs = LQ),
            ratio_uq = quantile(ratio, probs = UQ))
ymax <- max(temp_df$ratio_uq)*1.1
plot_list[["bias_srr"]] <- 
  ...
  theme(text = element_text(size = font_size), 
        ...
        legend.text = element_text(size = font_size),
        panel.grid.major = element_line(color = "grey80", size = 0.5))
ggsave(paste0(getwd(),"/Figs/MSE/",OM_name,"/bias_srr.jpeg"), plot=last_plot(), 
       width=width, height=height, units="in")
...
# Generate boxplots for short-term and long-term catch, F, SSB, SSB/SSBMSY, and AAVY...
stc <- plot_boxplot(y_var = "C", ylabel = "Catch (kt)")
ltc <- plot_boxplot(y_var = "C", ylabel = "Catch (kt)", period_chosen = "LT")
stf <- plot_boxplot(y_var = "FM", ylabel = "F")
ltf <- plot_boxplot(y_var = "FM", ylabel = "F", period_chosen = "LT")
stb <- plot_boxplot(y_var = "SSB", ylabel = "SSB (kt)")
ltb <- plot_boxplot(y_var = "SSB", ylabel = "SSB (kt)", period_chosen = "LT")
stbmsy <- plot_boxplot(y_var = "SSB_SSBMSYFirst5", ylabel = expression(SSB/SSB["MSY"]))
ltbmsy <- plot_boxplot(y_var = "SSB_SSBMSYFirst5", ylabel = expression(SSB/SSB["MSY"]), period_chosen = "LT")
sta <- plot_boxplot(data = boxplot_DF_AAVY, y_var = "value", ylabel = "AAVY (kt)")
lta <- plot_boxplot(data = boxplot_DF_AAVY, y_var = "value", ylabel = "AAVY (kt)", period_chosen = "LT")

# ...and then combine into a panel plot...
plot_list[["Boxplot"]] <- stbmsy + ggtitle("ST") + ltbmsy + ggtitle("LT") + stb + 
  ltb + stf + ltf + stc + ltc + sta + lta + 
  plot_layout(ncol = 2, axes = "collect") & 
  theme(plot.title = element_text(hjust = 0.5), 
        axis.text.x = element_text(angle = 90, vjust = 0.25, hjust=1, size = font_size))
ggsave(paste0(getwd(),"/Figs/MSE/",OM_name,"/boxplot.jpeg"), plot=last_plot(), 
       width=width_boxplot, height=height_boxplot, units="in")
...
plot_list[["Median_SSB_LRPs"]] <- 
  plot_ts(proj_df = PlotDFFilter,
          ...
          proj_col_uq = "UQSSB", ylab = "SSB (kt)", show_historical = TRUE, HCR_lab = "HCR") +
  geom_line(data = RP_PLOTS[RP_PLOTS$Yr>50,], 
            aes(x = Yr, y = value, linetype = timeperiod, group = name), 
            size = lwd*1.25, alpha = 0.6) + 
  scale_linetype_manual(values = c(1,2,3), 
                        labels = c("Full","Initial","Recent"), name = "LRP & USR")
