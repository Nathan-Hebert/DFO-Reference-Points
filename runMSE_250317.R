...
# Set up storage for control points and other quantities needed for HCRs
empty_matrix <- matrix(NA, nrow = OM@proyears, ncol = OM@nsim)
elements <- c("F_MSY_First5", "F_MSY_Last5", "F_MSY_EntireTS", 
              ...
              "q_Mixed","q_MixedBPA",
              "SSB_Sigma_MixedBPA")
CP_list <- setNames(replicate(length(elements), empty_matrix, 
                              simplify = FALSE), elements)
...
# Save DF and historical sim data (hist)
...
# Generate figures
source("generate_MSE_figs_250317.R")